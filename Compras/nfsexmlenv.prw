...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Private lDiscrCpt :=  SuperGetMV("MV_XNFSDC",,.F.)//parametro personalizado que indica que a discriminação será completa 
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				EndIf
				//----------------------------
				// INCLUSÃO DE TODOS OS CAMPOS DE DESCRIÇÃO, DISCRIMINAÇÃO E MENSAGEM 
				// TICKET:7085 - AUGUSTO MEIRELLES
				IF lDiscrCpt //parametro personalizado que indica que a discriminação será completa
					If !lNFeDesc
						If lNatOper .And. lSC5 .And. nCont == 1 .and. !Empty(cFieldMsg) .and. SC5->(FieldPos(cFieldMsg)) > 0 .and. !Empty(&("SC5->"+cFieldMsg))
							cNatOper := If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(&("SC5->"+cFieldMsg))),&("SC5->"+cFieldMsg))+" "
						ENDIF
						IF lNatOper .And. lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
							cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)),SC5->C5_MENNOTA)+" "
						ENDIF
						If SF2->(FieldPos("F2_MENNOTA")) <> 0 .and. !AllTrim(SF2->F2_MENNOTA) $ cMensCli .and. !Empty(AllTrim(SF2->F2_MENNOTA)) .and. !AllTrim(SF2->F2_MENNOTA) $ cDiscrNFSe
							cDiscrNFSe +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SF2->F2_MENNOTA)),AllTrim(SF2->F2_MENNOTA))
						EndIf //[ ] colocar regra para preencher cDiscrNFSe senao
					Else 
						If lSC5 .And. nCont == 1 .and. !Empty(cFieldMsg) .and. SC5->(FieldPos(cFieldMsg)) > 0 .and. !Empty(&("SC5->"+cFieldMsg))
							cDiscrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(&("SC5->"+cFieldMsg))),&("SC5->"+cFieldMsg))+" "
						ENDIF 
						If lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1 .and. !AllTrim(SC5->C5_MENNOTA) $ cDiscrNFSe
							cDiscrNFSe += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)),SC5->C5_MENNOTA)+" "
						ENDIF
						IF !Empty(AllTrim(SF2->F2_MENNOTA)) .And. nCont == 1 .and. !AllTrim(SF2->F2_MENNOTA) $ cDiscrNFSe
							cDiscrNFSe +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SF2->F2_MENNOTA)),AllTrim(SF2->F2_MENNOTA))
						EndIf
					EndIf
				endif
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf,aRetSF3)	
	...
	for nX := 1 to len( aProd )
		...
		// TICKET:7085 - AUGUSTO MEIRELLES
		IF lDiscrCpt //parametro personalizado que indica que a discriminação será completa
			//LINHA BASE: If ( SC6->(FieldPos("C6_DESCRI")) > 0 .And. Len(aProd[nX]) > 40 .And. !Empty(aProd[nX][41]) ) .And. (!lNFeDesc .And. !GetNewPar("MV_NFESERV","1") == "1" .And. !Empty(cCpmUsr) )
			If !( SC6->(FieldPos("C6_DESCRI")) > 0 .And. Len(aProd[nX]) > 40 .And. !Empty(aProd[nX][41]) ) 
				IF !(!lNFeDesc .And. !GetNewPar("MV_NFESERV","1") == "1" .And. !Empty(cCpmUsr) )
					cString	+= "<discr>" + AllTrim(aProd[nX][41])+ AllTrim(cNatOper) + AllTrim(cDiscrNFSe)+ cCargaTrb + "</discr>"				
				Else
