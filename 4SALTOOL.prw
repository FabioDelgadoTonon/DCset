...
@version 1.0
*/
User Function 4SALTOOL()
    Private cAliasMdl   := "" 
    Private aRotina     := {}

//Paramentros para Atalho: " -m -p=u_4SALTOOL -c=HOM_23608 -e=ABDHDU_HOM_DESENV_23608
// -m = Multi tela0
...
//Cria o MsApp
    ...
    oApp:bMainInit:= {|| MsgRun("4SALTOOL - Inicializando Ambiente de Desenvolvimento","Aguarde...",;
        {|| RpcSetEnv("22","00" , "marcio.fabritech", "fabritech@2022" ), }),;
        TIDEV(),;
   /*     Final("Manutenção Finalizada")*/}
...
Static Function TIDev()
	...
	If Type("OMAINWND") != "O"
        ...
        Private lSenha      := .T.
        Private oFont15		:= TFont():New( "Arial",,15,.T.,.F.,5,.T.,5,.T.,.F.)
        Private oFont15b	:= TFont():New( "Arial",,15,.T.,.T.,5,.T.,5,.T.,.F.)
...
Static Function MainDevApp(lInit)
	...
	Local aEmpExQr  := {}
	Local aFolder   := {'Query', 'Dicionário', 'Inspeção de Funções/Comandos', 'Linha de Comando','Html', 'File Explorer','Conversor Base64', "AxCadastro"}
	...
	DEFINE MSDIALOG oDlg FROM 0,0 TO 0,0 TITLE  "Ferramenta de desenvolvimento ( Ambiente: "+Alltrim(GetEnvServer()) + " / Server: " + AllTrim(GETSERVERIP()) +" )"
        oDlg:lEscClose := .F.
	    oDlg:SetCoors(oRect)
        ...
        oFolAx := TFolder():New(, , aSFolder7, aSFolder7, oFol:aDialogs[8], , , , .T., .F.)
	    oFolAx:Align := CONTROL_ALIGN_ALLCLIENT
        //--Folder Dicionario

        For nSF := 1 to 1
            FolderAx(oFolAx:aDialogs[nSF], oSize, nSF)
        Next
    ...
    oSysTray := TSystemTray():New(oMainWnd, "br_verde_mdi.png")
    oMenu := TMenu():New(0,0,0,0,.T.,,oMainWnd)
   
    oIt1  := TMenuItem():New(oMainWnd,'Menu Item 1',,,,{|| TIDEV() },,'AVGLBPAR1',,,,,,,.T.)
    oMenu:Add(oIt1)

    oSysTray:cToolTip := "CAOA"
    oSysTray:setMenu(oMenu)
	//oDlg:activate("HIDDEN")
...
Static Function FolderAx(oFol2, oSize, np)
    Local nLin:= 02
    Local nCol:= 02
	Local nB := oSize:GetDimension("PANEL", "LINEND")
	Local nR := oSize:GetDimension("PANEL", "COLEND")
    ...
    oPanelM1 :Align := CONTROL_ALIGN_TOP
        ...
        @ 02,025 GET cAliasDic of oPanelM1 SIZE 015,09 PIXEL PICTURE "@!"
        nCol := 50
        @ nLin, nCol        BUTTON oBut  PROMPT '&Executar'      SIZE 045,010 ACTION zBrowse(cAliasDic, oPnlDicI, np) OF oPanelM1 PIXEL; oBut:nClrText :=0
...
Static Function ModelDef()
    Local oModel
    Local oStru := FWFormStruct( 1, cAliasMdl )

    oModel := MPFormModel():New('TiToolMdl', , , {|| .t.} )
    oModel:AddFields( 'cmaster', /*cOwner*/, oStru)
    oModel:SetDescription( 'Model ', cAliasMdl )
...
Static Function ViewDef()
    ...
    Local oStru  := FWFormStruct( 2, cAliasMdl )

    oView:SetModel( oModel )
    oView:AddField( 'VIEW', oStru, 'cmaster' )
...
Static Function MenuDef()
    Local aRet := {}
    ADD OPTION aRet TITLE 'Visualizar'  ACTION 'VIEWDEF.4SALTOOL'   OPERATION MODEL_OPERATION_VIEW      ACCESS 0
    ADD OPTION aRet TITLE 'Incluir'     ACTION 'VIEWDEF.4SALTOOL'   OPERATION MODEL_OPERATION_INSERT    ACCESS 0
    ADD OPTION aRet TITLE 'Alterar'     ACTION 'VIEWDEF.4SALTOOL'   OPERATION MODEL_OPERATION_UPDATE    ACCESS 0
    ADD OPTION aRet TITLE 'Excluir'     ACTION 'VIEWDEF.4SALTOOL'   OPERATION MODEL_OPERATION_DELETE    ACCESS 0
Return aRet


Static Function zBrowse(cAlias, oPnlDicI, np)
	Local cAliasTst := MyNextAlias()
    Local bErroA 
    Local cErro := ""
    Local lTermino := .F.
	...
	If ValType(aFwBrw[np])=='O'
		aFwBrw[np]:DeActivate(.T.)
	EndIf
    ...
    Begin Sequence
        ...
        aFwBrw[np] := FWMBrowse():New(oPnlDicI)
        aRotina := MenuDef()
        aFwBrw[np]:SetAlias(cAliasMdl)
         
        aFwBrw[np]:Setdoubleclick({|| AxAltera(cAlias, (cAlias)->(Recno()), 3, aCps, aCps  )   })

        aFwBrw[np]:Activate(oPnlDicI)
        lTermino := .t.
...
Static Function PApQry2Run(cQuery, cAliasExqr, cAliasTst, oPnlQryI, np, oTime, oQtd)
    ...
    If ! cComando == "SELECT"
        ...
        zPSWUpd()
        If lSenha == .F.
            MsgInfo("Comando Bloqueado para esse usuário!","Registro lockado")
            Return
        EndIf
    EndIf
...
Static Function ExportExcel(cAliasTst, cPlan, cTit, lX3Titles, aHeadTitles, cTotalCols, aFieldPict)  
    Local cFile := ""
    Local oExcel
    Private lAbortPrint := .F.
    ...
    Processa({|| oExcel := ProcExcel(oExcel, cAliasTst, cPlan, cTit, lX3Titles, aHeadTitles, cTotalCols, aFieldPict)},,,.T.) 
...
Static Function ProcExcel(oExcel, cAliasCSV, cPlan, cTit, lX3Titles, aHeadTitles, cTotalCols, aFieldPict)  
    Local nx      := 0
    Local nReg    := 0
    Local aFields := (cAliasCSV)->(dbStruct())
    Local nFields := Len(aFields)
    ...
    For nx:= 1 to nFields
        ...
        ElseIf aFields[nx, 2] == "N"
			...
			nFormat	:= 2 
            // lTotal  := .T.
            If ValType(cTotalCols) != "C" .OR. ;
                ( !Empty(cTotalCols) .AND. AllTrim(aFields[nX, 1]) $ cTotalCols )
                lTotal  :=  .T.
            EndIf
        Else
			nAlign 	:= 1
        ...
        If lX3Titles
            cTitCol := FwX3Titulo(aFields[nX, 1])
        ElseIf !Empty(aHeadTitles) .And. nX <= Len(aHeadTitles)
            cTitCol := aHeadTitles[nX, 1] // {"Titulo","CAMPO"}
        // Else
        //     cTitCol := aFields[nX, 1]
        EndIf
        oExcel:AddColumn(cPlan, cTit, cTitCol, nAlign, nFormat, lTotal)  
    Next
    ...
    While (cAliasCSV)->(! Eof())
        ...
        For nx:= 1 to nFields
            ...
            If ValType(aFieldPict) == "A" .AND. !Empty(aFieldPict[nx][1])
                aadd(aLinha, Transform( (cAliasCSV)->(FieldGet(nx)) , aFieldPict[nx][1] ) )
            Else
                aadd(aLinha, (cAliasCSV)->(FieldGet(nx)) )
            EndIf
        Next
...
Static Function OpenBtn(cAtual,cOnde)
	Local cRetDir := ""
	If cOnde == "T"
		cRetDir := cGetFile("Todos Arquivos|*.*|","Escolha o caminho dos arquivos.")//,0,cAtual,,GETF_RETDIRECTORY+GETF_LOCALHARD+GETF_LOCALFLOPPY+GETF_NETWORKDRIVE,.T.)
	ElseIf cOnde == "S"
		cRetDir := cGetFile("Todos Arquivos|*.*|","Escolha o caminho dos arquivos.")//,0,cAtual,,GETF_RETDIRECTORY+GETF_ONLYSERVER,.T.)
	EndIf
...
Static Function DicTst(cAliasDic, oPnlDicI)
    ...
    While   SX3->(! Eof() .and. SX3->X3_ARQUIVO == cAliasDic)
        If ! SX3->X3_CONTEXT == 'R'
            SX3->(dbSkip())
            Loop
        EndIf
...
Static Function Tools(cQry,oOwner, np)
	...
	Local nX := 0
	Local oMenu,oIte1,oIte4,oIte4_1 //oMenu,oIte1,oIte2,oIte3,oIte4,oIte4_1
	Local cAux:= ''
	Local bBlock:={||}
...
Static Function SelEmp(__cEmp, __cFil)
    Local oDlgLogin, oCbxEmp, oFont //oBtnOk, oBtnCancel, oFont
    Local cEmpAtu			:= ""
    Local lOk				:= .F.
    Local aCbxEmp			:= {}
...
/*/
//----------------------------------------------------------
Static Function zPSWUpd()

Private aAdvSize	 := MsAdvSize()
Private aInfoAdvSize := {aAdvSize[1],aAdvSize[2],aAdvSize[3],aAdvSize[4],0,0}
Private aObjCoords	 := {{000,000,.T.,.F.},{000,000,.T.,.T.}}
Private aObjSize	 := MsObjSize(aInfoAdvSize,aObjCoords)
Private nLinIni		 := 10
Private nColIni		 := 10
Private nLinFin		 := 200
Private nColFin		 := 350
...
Private oSenha2
Private cSenha2		 := Space(06)
Private cSenhaOk	:= "098623"

RETURN .T.
DEFINE MSDIALOG oDlgX TITLE "Senha de Autenticação" FROM nLinIni,nColIni TO nLinFin,nColFin+150 PIXEL

@ 060,050 Say "Senha:"				Size (060),(010)			PIXEL OF oDlgX//FONT oFont15 COLOR CLR_BLUE PIXEL OF oDlgX
@ 060,0100 MsGet oSenha2	Var cSenha2 Size (050),(010) PASSWORD   PIXEL OF oDlgX//FONT oFont15 COLOR CLR_RED  PIXEL OF oDlgX

ACTIVATE MSDIALOG oDlgX CENTERED ON INIT EnchoiceBar(oDlgX,{|| U_FBVLDPAS() },{|| U_FBCLSPAS()})
...
User Function FBCLSPAS()

    MsgInfo("Comando cancelado","Registro lockado")
